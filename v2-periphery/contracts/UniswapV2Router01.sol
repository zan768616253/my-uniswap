...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            // last swap -> send token out to "to" address
            // otherwise, send token out to the next pair
            address to = i < path.length - 2 ? UniswapV2Library.pairFor(factory, output, path[i + 2]) : _to;
            // swap doesn't ask for amount in, only amount out
    ...
    function swapExactTokensForTokens(
        ...
        // amounts[0] = input, amounts[last] = output, amounts[1..last-1] = intermediate amounts
        amounts = UniswapV2Library.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        // create2 pair.address
        // directlly transfer input token to pair
